...
- **Dual-agent architecture: MerchantAgent + BuyerAgent, each Hermes-style.** The MerchantAgent proposes offers (bandit + optional LLM reasoner); the BuyerAgent negotiates over ACP and accepts/declines independently. Orchestration is LangGraph `StateGraph`. Both agents have their own harness, eval, and graph state.
- **The LLM is allowed only as an advisory reasoner, never on the money path.** Reasoning/explanation in `reasoning/` may use an LLM (Hermes-style AIAgent loop with read-only tools), but it is structurally prevented from affecting money actions: read-only tools only, it writes only to `reasoning_log`, and it never proposes or executes a settlement. The decision layer (bandit) and the eval harness stay non-LLM by design. If a task needs an LLM to *score, parse, or decide* a money action, that's a signal to reconsider the approach. The hard constraints — the rule layer always wins, the immutable core contract, the audit trail — guarantee the LLM cannot violate the rules regardless of its output.
